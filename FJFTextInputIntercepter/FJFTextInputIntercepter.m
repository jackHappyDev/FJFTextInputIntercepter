...
- (void)textFieldTextDidChangeWithNotification:(NSNotification *)noti {
    ...
    if (finalText.length > 0) {
        textField.text = finalText;
            ...
            self.intercepterNumberType == FJFTextInputIntercepterNumberAndLetter ||
            self.intercepterNumberType == FJFTextInputIntercepterNoChinese ||
...
- (NSString *)handleWithInputText:(NSString *)inputText {
    ...
    //只允许输入字母和数字
    else if (self.intercepterNumberType == FJFTextInputIntercepterNumberAndLetter){
        if ([tmpReplacementString fjf_isCertainStringType:FJFTextInputStringTypeNumberAndLetter] == NO) {
            inputText = _previousText;
        }
    }
    
    //不允许输入中文汉字
    else if (self.intercepterNumberType == FJFTextInputIntercepterNoChinese){
        if ([tmpReplacementString fjf_isCertainStringType:FJFTextInputStringTypeNoChinese] == NO) {
            inputText = _previousText;
        }
    }
